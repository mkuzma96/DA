            ...
            for mdrf in hyperpar_list['max_depth_rf']:
                for nfrf in hyperpar_list['n_features_rf']:
                    for mslrf in hyperpar_list['min_samples_leaf_rf']:
                        ...
                        Y_pred = np.empty((data_val_a.shape[0], data_val_a.shape[1]))
                        for i in range(data_val_a.shape[1]):
                            Y_pred[:,i] =RF_pred(np.concatenate([data_val_a[:,i:(i+1)], data_val_x], axis=1), model)
                        Y_true = data_val_y
                        loss = RMISE(Y_pred, Y_true)
                                                    ...
                                                    for mdrf in hyperpar_list['max_depth_rf']:
                                                        for nfrf in hyperpar_list['n_features_rf']:
                                                            for mslrf in hyperpar_list['min_samples_leaf_rf']:
                                                                ...
                                                                model = RF(data_train_scw, hyperpars)  
                                                                Y_pred = np.empty((data_val_a.shape[0], data_val_a.shape[1]))
                                                                for i in range(data_val_a.shape[1]):
                                                                    Y_pred[:,i] = RF_pred(np.concatenate([data_val_a[:,i:(i+1)], data_val_bae_x], axis=1), model)
                                                                Y_true = data_val_y
                                                                loss = RMISE(Y_pred, Y_true)
