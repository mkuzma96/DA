...
params_CF = {
    ...
    'max_depth_cf': None,
    'n_features_cf': 5,
    'min_samples_split_cf': 10,
    'min_samples_leaf_cf': 5
}

def CF(data, params):
    
    n_trees = params['n_trees_cf']
    max_depth = params['max_depth_cf']
    n_features = params['n_features_cf']
    min_samples_leaf = params['min_samples_leaf_cf']
        
    model = CausalForest(n_estimators=n_trees, max_depth=max_depth,
                         min_samples_leaf=min_samples_leaf, max_features=n_features)
    
    model.fit(data[:,2:], data[:,1], data[:,0])
    
    return model

# CF fit:
# mod_CF = CF(data_rw, params_CF)
...
def CF_pred(newdata, model):
    
    theta_x = model.predict(newdata[:,1:]).reshape(-1)
    Y_pred = theta_x*newdata[:,0]
    
    return Y_pred

# CF predict:
# Y_pred_CF = CF_pred(data_rw[:,1:], mod_CF)
