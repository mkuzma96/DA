...
#%% Simulated data test - factual outcome based on HIV 2017
...
for i in range(p):
    data_interaction[:,i] = A*data_interaction[:,i]
means_f = intercept + np.matmul(np.concatenate([data_test[:,1:], data_interaction], axis=1), coefs)
means_f = (means_f - np.min(means_f))/(np.max(means_f) - np.min(means_f))
means_f = np.sqrt(means_f)
Y_f = np.random.normal(loc=means_f, scale=0.01, size=(n,1))
data_sim_test_f = np.concatenate([Y_f, data_test[:,1:]],axis=1)
data_sim_test_f = pd.DataFrame(data_sim_test_f)
# data_sim_test_f.to_csv('Sim_data_test_f.csv', index=False)
...
# Prepare semi-synthetic data
A_cf = np.random.uniform(np.min(data_test[:,1]), np.max(data_test[:,1]), (n,1))
data_interaction = data_test[:,2:].copy()
for i in range(0,p):
    data_interaction[:,i] = A_cf.reshape((n,))*data_interaction[:,i]
means_cf = intercept + np.matmul(np.concatenate([A_cf, data_test[:,2:], data_interaction], axis=1), coefs)
means_cf = (means_cf - np.min(means_cf))/(np.max(means_cf) - np.min(means_cf))
means_cf = np.sqrt(means_cf)
Y_cf = np.random.normal(loc=means_cf, scale=0.01, size=(n,1))
data_sim_test_cf = np.concatenate([Y_cf, A_cf, data_test[:,2:]],axis=1)
data_sim_test_cf = pd.DataFrame(data_sim_test_cf)
# data_sim_test_cf.to_csv('Sim_data_test_cf.csv', index=False)
